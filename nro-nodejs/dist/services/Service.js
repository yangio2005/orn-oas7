...
class Service {
    ...
    setClientType(session, msg) {
        Logger_1.Logger.debug(`setClientType Body: ${msg.getData().toString('hex')}`);
        try {
            ...
            Logger_1.Logger.debug(`Client Info: Type=${typeClient}, Zoom=${zoomLevel}, Res=${width}x${height}, Platform=${platform}`);
    ...
    messageSubCommand(command) {
        const msg = new Message_1.Message(Cmd_1.Cmd.SUB_COMMAND); // Cmd.SUB_COMMAND is -30
        msg.writer.writeByte(command);
        return msg;
    }
    player(player) {
        ...
        try {
            ...
            Logger_1.Logger.error("Error sending player info: " + e);
        }
        finally {
            msg.cleanup();
        }
    ...
    point(player) {
        if (!player || !player.session)
            return;
        const msg = new Message_1.Message(-42);
        try {
            ...
            Logger_1.Logger.error("Error sending point info: " + e);
        }
        finally {
            msg.cleanup();
        }
    ...
    Send_Info_NV(player) {
        ...
        try {
            ...
            Logger_1.Logger.error("Error sending Info_NV: " + e);
        }
        finally {
            msg.cleanup();
        }
    }
    Send_Caitrang(player) {
        // TODO: Implement send caitrang
    }
    sendFlagBag(player) {
        // TODO: Implement send flag bag
    }
    sendCaption(session, gender) {
        const msg = new Message_1.Message(-41);
        try {
            ...
            Logger_1.Logger.error("Error sending caption: " + e);
        }
        finally {
            msg.cleanup();
        }
     ...
     */
    sendMessage(session, cmd, filename) {
        const msg = new Message_1.Message(cmd);
        try {
            // In Java, this reads a cached message file and sends it
            // For now, just send empty message with the command
            session.sendMessage(msg);
            Logger_1.Logger.debug(`Sent cached message: ${cmd} (${filename})`);
        }
        catch (e) {
            Logger_1.Logger.error(`Error sending message ${cmd}: ` + e);
        }
        finally {
            msg.cleanup();
        }
