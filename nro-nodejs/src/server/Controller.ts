...
export class Controller {
     ...
     * Java: line 66-617
     */
    public onMessage(session: Session, msg: Message): void {
        ...
        try {
            switch (cmd) {
                ...
                case -111: // GET_IMAGE_SOURCE2
                    DataGame.sendDataImageVersion(session);
                    break;

                // Java: line 591-592
                case Cmd.NOT_LOGIN: // -29
                ...
                case -74: // GET_IMAGE_SOURCE
                    ...
                    if (type === 1) {
                        DataGame.sendSizeRes(session);
                    } else if (type === 2) {
                        DataGame.sendRes(session);
                    }
                    break;
                ...
                case 11: // REQUEST_MAPTEMPLATE
                    ...
                    DataGame.requestMobTemplate(session, modId);
                    break;

                // Java: line 389-395
                case 66: // GET_IMG_BY_NAME
                    DataGame.sendImageByName(session, msg.reader.readUTF());
                    break;

                // Java: line 396-406
                case -66: // GET_EFFDATA
                    const effId = msg.reader.readShort();
                    DataGame.effData(session, effId);
                    break;

                // Java: line 417-420
                case -32: // BACKGROUND_TEMPLATE
                    const bgId = msg.reader.readShort();
                    DataGame.sendItemBGTemplate(session, bgId);
                    break;
                ...
                case -81: // COMBINNE
                    if (session.player) {
                        ...
                        Logger.debug(`Combine request with ${indexItemCount} items`);
                    }
                    break;
                ...
                case -67: // REQUEST_ICON
                    if (!(session as any).isRIcon) {
                        const iconId = msg.reader.readInt();
                        DataGame.sendIcon(session, iconId);
                    }
                    break;
                ...
                case 44: // CHAT_MAP
                    if (session.player) {
                        ...
                        Logger.debug(`Chat: ${session.player.name}: ${chatText}`);
                    }
                    break;
                ...
                case 32: // MENU
                    if (session.player) {
                        ...
                        Logger.debug(`Menu select: NPC ${npcId}, option ${select}`);
                    }
                    break;
                ...
                case 33: // OPEN_UI_MENU
                    if (session.player) {
                        ...
                        Logger.debug(`Open menu: NPC ${npcId}`);
                    }
                    break;
                ...
                case 34: // SKILL_SELECT
                    if (session.player) {
                        ...
                        Logger.debug(`Select skill: ${selectSkill}`);
                    }
                    break;
                ...
                case 54: // PLAYER_ATTACK_NPC
                    if (session.player) {
                        ...
                        Logger.debug(`Attack mob: ${mobIndex}`);
                    }
                    break;
                ...
                case -60: // PLAYER_ATTACK_PLAYER
                    if (session.player) {
                        ...
                        Logger.debug(`Attack player: ${playerId}`);
                    }
                    break;
                ...
                case -20: // ITEMMAP_MYPICK
                    if (session.player) {
                        ...
                        Logger.debug(`Pick item: ${itemMapId}`);
                    }
                    break;
                ...
                case -7: // PLAYER_MOVE
                    if (session.player) {
                        ...
                        Logger.debug(`Player move to (${toX}, ${toY})`);
                    }
                    break;
                ...
                case 21: // ZONE_CHANGE
                    if (session.player) {
                        ...
                        Logger.debug(`Change zone: ${zoneId}`);
                    }
                    break;
                ...
                case -71: // CHAT_THEGIOI_CLIENT
                    if (session.player) {
                        ...
                        Logger.debug(`Global chat: ${globalChat}`);
                    }
                    break;
                ...
                case 6: // ITEM_BUY
                    if (session.player) {
                        ...
                        Logger.debug(`Buy item: type=${typeBuy}, id=${tempId}, qty=${quantity}`);
                    }
                    break;
                ...
                case 7: // ITEM_SALE
                    if (session.player) {
                        ...
                        Logger.debug(`Sell item: action=${action}`);
                    }
                    break;
                ...
                case -23: // MAP_CHANGE
                    if (session.player) {
                        ...
                        Logger.debug("Change map waypoint");
                    }
                    break;
            ...
            Logger.error(`Error handling message ${cmd}: ${e}`);
        } finally {
            msg.cleanup();
        }
    }
     ...
     * Java: line 619-645
     */
    private messageNotLogin(session: Session, msg: Message): void {
        try {
            ...
            switch (cmd) {
                case 0: // LOGIN
                    try {
                        ...
                        AuthService.getInstance().login(session, username, password);
                    } catch (e) {
            ...
            Logger.error("Error in messageNotLogin: " + e);
        }
    }
    ...
    private messageNotMap(session: Session, msg: Message): void {
        try {
            ...
            switch (cmd) {
                ...
                case 6: // UPDATE_MAP
                    DataGame.updateMap(session);
                    break;

                case 7: // UPDATE_SKILL
                    DataGame.updateSkill(session);
                    break;
                ...
                case 10: // REQUEST_MAPTEMPLATE
                    ...
                    DataGame.sendMapTemp(session, mapId);
                    break;
            ...
            Logger.error("Error in messageNotMap: " + e);
        }
    }
     ...
     * Java: line 701-727
     */
    private messageSubCommand(session: Session, msg: Message): void {
        try {
            ...
            switch (command) {
                case 16: // POTENTIAL_UP
                    ...
                    if (session.player) {
                        ...
                        Logger.debug(`Increase point: type=${type}, point=${point}`);
                    }
                    break;
                ...
                case 64: // PLAYER_MENU_ACTION
                    ...
                    if (session.player) {
                        ...
                        Logger.debug(`Player menu action: player=${playerId}, menu=${menuId}`);
                    }
                    break;
            ...
            Logger.error("Error in messageSubCommand: " + e);
        }
    }
     ...
     * Java: line 729-774
     */
    private createChar(session: Session, msg: Message): void {
        try {
            const name = msg.reader.readUTF();
            const gender = msg.reader.readByte();
            const hair = msg.reader.readByte();

            Logger.info(`Creating character: ${name}, gender: ${gender}, head: ${hair}`);
            AuthService.getInstance().createCharacter(session, name, gender, hair);
        } catch (e) {
            Logger.error("Error in createChar: " + e);
        }
    }
